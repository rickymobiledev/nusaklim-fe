...
export function useWindSpeedChart(
  stationIds: string[],
  ...
  return useQuery({
    queryKey: ["wind-speed-daily", stationIds, startDate, endDate],
    queryFn: () => {
      ...
      return fetchJson<ApiListResponse<WindSpeedStationSeries>>(
        `/api/wind-speed/daily?${qs}`,
      );
    },
    select: (res) => res.data,
