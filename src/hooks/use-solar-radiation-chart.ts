...
export function useSolarRadiationChart(
  stationIds: string[],
  ...
  return useQuery({
    queryKey: ["solar-radiation-daily", stationIds, startDate, endDate],
    queryFn: () => {
      ...
      return fetchJson<ApiListResponse<SolarRadiationStationSeries>>(
        `/api/solar-radiation/daily?${qs}`,
      );
    },
    select: (res) => res.data,
