...
export function useAirPressureChart(
  stationIds: string[],
  ...
  return useQuery({
    queryKey: ["air-pressure-daily", stationIds, startDate, endDate],
    queryFn: () => {
      ...
      return fetchJson<ApiListResponse<AirPressureStationSeries>>(
        `/api/air-pressure/daily?${qs}`,
      );
    },
    select: (res) => res.data,
