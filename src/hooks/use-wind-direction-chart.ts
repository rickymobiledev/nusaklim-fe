...
export function useWindDirectionChart(
  stationIds: string[],
  ...
  return useQuery({
    queryKey: ["wind-direction-daily", stationIds, startDate, endDate],
    queryFn: () => {
      ...
      return fetchJson<ApiListResponse<WindDirectionStationSeries>>(
        `/api/wind-direction/daily?${qs}`,
      );
    },
    select: (res) => res.data,
