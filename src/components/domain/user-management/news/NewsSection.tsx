...
import {
  ...
  useUpdateFeatured,
  useUpdateStatus,
} from "@/hooks/use-news-management";
import { media } from "@/lib/breakpoints";
import type { NewsItem } from "@/types/domain";
import { NewsDetailDialog } from "./NewsDetailDialog";
import { NewsListWithConfirm } from "./NewsList";
...
export function NewsSection() {
  ...
  /** Semua mutation — untuk `isPending` ke ConfirmDialog. */
  const isPending =
    deleteNews.isPending || updateFeatured.isPending || updateStatus.isPending;
  ...
  function handleDeleteFromDetail(item: NewsItem) {
    setDetailTarget(null);
    // Konfirmasi diurus NewsListWithConfirm — tidak perlu state delete terpisah.
    // Panggil delete langsung karena sudah melewati confirm dialog di sana.
    ...
    <Wrapper>
      ...
      >
        <NewsListWithConfirm
          ...
          onConfirmDelete={(item) => deleteNews.mutate(item.id)}
          onConfirmFeatured={(item, isFeatured) =>
            updateFeatured.mutate({ id: item.id, isFeatured })
          }
          onConfirmStatus={(item, status) =>
