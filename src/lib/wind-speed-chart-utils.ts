 ...
 *  bentuk yang Recharts `<BarChart data={...}>` butuhkan supaya semua
 *  `<Bar dataKey={stationId}>` berbagi sumbu-X yang sama. */
export function mergeSeriesByDate(series: WindSpeedStationSeries[]): MergedChartRow[] {
 ...
 *  — dipakai tombol "Unduh Data" di `WindSpeedFilters.tsx`. Kolom ikut
 *  urutan `series` (= urutan stasiun terpilih), baris ikut tanggal
 *  gabungan dari `mergeSeriesByDate()` biar konsisten sama data yang
 *  dipakai chart. */
export function buildWindSpeedCsv(series: WindSpeedStationSeries[]): string {
