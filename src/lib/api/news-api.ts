...
export interface NewsApi {
  /** Daftar berita publik dari `GET /news/published` — dipakai di Beranda
   *  ("Berita Pilihan") dan halaman daftar berita publik. Tidak ada
   *  `company_code`, tidak ada filter status (endpoint BE sudah published
   *  saja), `is_featured = 1` di paling depan. */
  getPublishedNews(): Promise<ApiListResponse<NewsItem>>;
  ...
  /** `POST /news/update_featured` — set `is_featured` berita. */
  updateFeatured(id: string, isFeatured: boolean): Promise<{ message?: string }>;
  /** `POST /news/update_status` — set status: `draft | published | archived`. */
  updateStatus(
    id: string,
