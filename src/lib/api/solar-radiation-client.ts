...
import { fetchRadiationRange } from "./weather-daily-client";

/** Server-only, dipanggil HANYA dari `app/api/solar-radiation/daily/route.ts`
 *  — bukan domain mock/real toggle spt `lib/api/index.ts` (halaman
 *  `/solar-radiation` belum tercantum di daftar domain CLAUDE.md, jadi
...
export async function getRadiationSeries(
  stationIds: string[],
  startDate: Date,
  endDate: Date,
  companyId?: string,
): Promise<ApiListResponse<SolarRadiationStationSeries>> {
  ...
  try {
    const series = await Promise.all(
      stations.map(async (station) => ({
        ...
        points: await fetchRadiationRange(station.id, startDate, endDate, companyId),
      })),
    );
