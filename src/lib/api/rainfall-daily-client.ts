...
import { fetchRainfallRange } from "./weather-daily-client";

/** Server-only, dipanggil HANYA dari `app/api/rainfall/daily/route.ts` —
 *  bukan domain mock/real toggle spt `lib/api/index.ts` (halaman
 *  `/rainfall` belum tercantum di daftar domain CLAUDE.md, jadi ini helper
 *  internal seperti `air-temperature-client.ts`, bukan `XxxApi` interface
 *  baru).
 *
...
export async function getRainfallSeries(
  ...
  endDate: Date,
  companyId?: string,
): Promise<ApiListResponse<RainfallStationSeries>> {
  ...
  try {
    const series = await Promise.all(
      stations.map(async (station) => ({
        ...
        points: await fetchRainfallRange(station.id, startDate, endDate, companyId),
      })),
    );
