...
import type { WindSpeedStationSeries } from "@/types/domain";
import { stationApi } from "./station-client";
import { fetchWindSpeedRange } from "./weather-daily-client";

/** Server-only, dipanggil HANYA dari `app/api/wind-speed/daily/route.ts` —
 *  bukan domain mock/real toggle spt `lib/api/index.ts` (halaman
 *  `/wind-speed` belum tercantum di daftar domain CLAUDE.md, jadi ini
 *  helper internal seperti `rainfall-daily-client.ts`, bukan `XxxApi`
 *  interface baru).
 *
...
export async function getWindSpeedSeries(
  stationIds: string[],
  startDate: Date,
  endDate: Date,
  companyId?: string,
): Promise<ApiListResponse<WindSpeedStationSeries>> {
  ...
  try {
    const series = await Promise.all(
      stations.map(async (station) => ({
        ...
        points: await fetchWindSpeedRange(station.id, startDate, endDate, companyId),
      })),
    );
