...
import { fetchWindDirectionRange } from "./weather-daily-client";

/** Server-only, dipanggil HANYA dari
 *  `app/api/wind-direction/daily/route.ts` — bukan domain mock/real toggle
 *  spt `lib/api/index.ts` (halaman `/wind-direction` belum tercantum di
 *  daftar domain CLAUDE.md, jadi ini helper internal seperti
 *  `wind-speed-daily-client.ts`, bukan `XxxApi` interface baru).
 *
...
export async function getWindDirectionSeries(
  stationIds: string[],
  startDate: Date,
  endDate: Date,
  companyId?: string,
): Promise<ApiListResponse<WindDirectionStationSeries>> {
  ...
  try {
    const series = await Promise.all(
      stations.map(async (station) => ({
        ...
        points: await fetchWindDirectionRange(station.id, startDate, endDate, companyId),
      })),
    );
