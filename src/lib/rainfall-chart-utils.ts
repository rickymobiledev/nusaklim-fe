import type { RainfallStationSeries } from "@/types/domain";
...
export function mergeSeriesByDate(series: RainfallStationSeries[]): MergedChartRow[] {
 ...
 *  — dipakai tombol "Unduh Data" di `RainfallFilters.tsx`. Kolom ikut
 *  urutan `series` (= urutan stasiun terpilih), baris ikut tanggal
 *  gabungan dari `mergeSeriesByDate()` biar konsisten sama data yang
 *  dipakai chart. */
export function buildRainfallCsv(series: RainfallStationSeries[]): string {
