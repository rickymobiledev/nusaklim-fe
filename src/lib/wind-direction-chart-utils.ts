...
export function mergeSeriesByDate(
  series: WindDirectionStationSeries[],
): MergedChartRow[] {
 ...
 *  — dipakai tombol "Unduh Data" di `WindDirectionFilters.tsx`. Kolom ikut
 *  urutan `series` (= urutan stasiun terpilih), baris ikut tanggal
 *  gabungan dari `mergeSeriesByDate()` biar konsisten sama data yang
 *  dipakai chart. */
export function buildWindDirectionCsv(series: WindDirectionStationSeries[]): string {
